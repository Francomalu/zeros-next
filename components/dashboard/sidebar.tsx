...
import { Bus, Calendar, ChevronDown, ChevronLeft, ChevronRight, CreditCard, LogOut, MapPin, Menu, Settings, User, UserCheck, Users, Wrench } from 'lucide-react';
...
export function Sidebar() {
  ...
  const filteredMenu = menuItems
    ...
    .map((item) => {
      if (item.submenu) {
        const filteredSubmenu = item.submenu.filter((sub) => !sub.roles || sub.roles.includes(userRole));
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className={cn('flex items-center p-4 border-b', isCollapsed ? 'justify-center' : 'gap-2')}>
            ...
            {!isCollapsed && <span className="text-xl font-bold text-blue-800 font-display">Zeros Tour</span>}
          </div>
                    ...
                    <div>
                      <button
                        ...
                        className={cn(
                          ...
                          expandedMenus[item.name.toLowerCase()] ? 'bg-blue-50 text-blue-700' : 'text-gray-700',
                      ...
                      >
                        ...
                        {!isCollapsed && <ChevronDown className={cn('h-4 w-4 transition-transform', expandedMenus[item.name.toLowerCase()] ? 'rotate-180' : '')} />}
                      </button>
                      {expandedMenus[item.name.toLowerCase()] && !isCollapsed && (
                            ...
                            <li key={subitem.name}>
                              <Link
                                ...
                                className={cn(
                                  ...
                                  pathname === subitem.path ? 'bg-blue-100 text-blue-700 font-medium' : 'text-gray-700'
                                )}
          ...
          <div className={cn('p-4 border-t', isCollapsed ? 'flex justify-center' : '')}>
            <Button
              variant="outline"
              className={cn('text-red-600 border-red-200 hover:bg-red-50 hover:text-red-700', isCollapsed ? 'w-10 h-10 p-0' : 'w-full justify-start')}
