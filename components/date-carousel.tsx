...
export function DateCarousel({ selectedDate, onDateSelect, className }: DateCarouselProps) {
      ...
      <div className="flex items-center">
        ...
        >
          {dates.map((date) => {
            ...
            const isToday = isEqual(date, new Date(new Date().setHours(0, 0, 0, 0)))
            ...
            return (
              ...
              >
                ...
                <span className={cn("text-lg font-bold", isSelected ? "text-white" : "text-blue-900")}>
                  {format(date, "d")}
                </span>
                <span className="text-xs">{isToday ? "Today" : format(date, "MMM")}</span>
              </button>
            )
          })}
        </div>
